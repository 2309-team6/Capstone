...
const { createComment, getComments, deleteComment, getCommentById, updateComment } = require('../db/comments');
...
commentsRouter.post("/", async (req, res, next) => {
  try {
    ...
    if (!content) {
      return res.status(400).json({ message: "Contents are required for comments"});
    }
